...
DeBruijinGraph::DeBruijinGraph(string S) {
    int k = 25;
    ...
    pair<vector<Node*>, vector<Edge>> graph_edges = DeBruijinAlgorithms::create_compress_graph(k, wta, lcp);

    this->vertices = graph_edges.first;
    this->edges = graph_edges.second;

    DeBruijinAlgorithms::finishGraphA1(this->vertices, this->edges, csa);
